...
func main() {
	...
	go func() {
		for {
			...
			case <-done:
				return
			default:
				fmt.Printf("\rGesamtfortschritt: %d Passwörter getestet", atomic.LoadInt64(&testedCounter))
	...
	if atomic.LoadInt32(&foundFlag) == 1 {
		...
		fmt.Printf("Getestete Passwörter: %d\n", atomic.LoadInt64(&testedCounter))
		fmt.Printf("Durchschnittliche Geschwindigkeit: %.0f Passwörter/Sekunde\n", float64(atomic.LoadInt64(&testedCounter))/elapsedTime)
	} else {
